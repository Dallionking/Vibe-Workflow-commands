...
export class VibeLearnCommand extends BaseCommand {
  ...
  private categorizeError(error: string): string {
    ...
    if (error.includes('not found')) return 'missing-resource';
  ...
  private generatePatternTags(pattern: any): string[] {
    ...
    if (pattern.confidence > 90) tags.push('high-confidence');
    if (pattern.usage?.count > 10) tags.push('frequently-used');
    if (pattern.autoGenerated) tags.push('auto-generated');
