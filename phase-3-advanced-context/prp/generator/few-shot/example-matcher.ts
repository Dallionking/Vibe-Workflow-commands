...
export class IntelligentExampleMatcher implements ExampleMatcher {
    ...
    private calculateDiversityScore(
        ...
        if (selectedExamples.length === 0) return 1.0;
    ...
    private calculateSelectionDiversity(examples: ExampleMatch[]): number {
        if (examples.length <= 1) return 1.0;
    ...
    private calculateRecencyScore(lastUsed?: Date): number {
        ...
        if (daysSinceUsed < 7) return 1.0;
        if (daysSinceUsed < 30) return 0.8;
        if (daysSinceUsed < 90) return 0.6;
        if (daysSinceUsed < 365) return 0.4;
        return 0.2;
    }
