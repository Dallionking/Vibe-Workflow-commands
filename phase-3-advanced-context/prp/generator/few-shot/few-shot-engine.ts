...
export class FewShotLearningEngine {
  ...
  private validateExample(example: FewShotExample): { isValid: boolean; issues: string[] } {
    ...
    if (!example.id) {
      issues.push('Missing example ID');
    }
    if (!example.input || example.input.length < 10) {
      issues.push('Input too short or missing');
    }
    if (!example.output || example.output.length < 10) {
      issues.push('Output too short or missing');
    }
    if (!example.reasoning || example.reasoning.length === 0) {
      issues.push('Missing reasoning steps');
    }
    if (!example.context.domain) {
  ...
  private analyzeLearningTrends(): { improving: number; declining: number; stable: number } {
    ...
    recentFeedback.forEach(feedback => {
      if (feedback.effectiveness > 0.7) {
        improving++;
      } else if (feedback.effectiveness < 0.4) {
        declining++;
      } else {
        stable++;
      }
