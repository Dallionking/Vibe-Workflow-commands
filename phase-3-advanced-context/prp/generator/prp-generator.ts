...
export class PRPGenerator {
    ...
    private determineContextDepth(analysis: any): 'shallow' | 'medium' | 'deep' {
        if (analysis.complexity > 7) return 'deep';
        if (analysis.complexity > 4) return 'medium';
        return 'shallow';
    }
    
    private selectReasoningPattern(analysis: any): 'chain-of-thought' | 'few-shot' | 'zero-shot' | 'self-consistency' {
        if (analysis.complexity > 6) return 'chain-of-thought';
        if (analysis.topics.length > 3) return 'few-shot';
        return 'zero-shot';
    }
    ...
    private determinePriority(context: PRPGenerationContext): 'low' | 'medium' | 'high' | 'critical' {
        if (context.currentPhase <= 3) return 'high';
        if (context.currentPhase <= 6) return 'medium';
        return 'low';
    }
