...
export class PRPGeneratorHelpers {
  ...
  static async analyzeReasoningComplexity(content: string): Promise<number> {
    ...
    if (content.includes('distributed') || content.includes('concurrent')) {
      complexity += 2;
    }
    ...
    if (content.includes('integration') || content.includes('compatibility')) {
      complexity += 1;
    }
    if (content.includes('security') || content.includes('authentication')) {
      complexity += 2;
    }
    if (content.includes('scale') || content.includes('scalability')) {
      complexity += 2;
    }
