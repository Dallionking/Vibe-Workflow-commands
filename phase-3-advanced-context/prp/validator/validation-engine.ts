...
export class PRPValidationEngine {
    ...
    private async validateOutputFormat(
        ...
        if (!outputFormat.structure || !outputFormat.structure.sections) {
            issues.push({
                type: 'warning',
                severity: 'medium',
    ...
    private calculateContentCompleteness(content: PRPContent): number {
        ...
        if (content.systemPrompt) filledFields++;
        if (content.contextPrompt) filledFields++;
        if (content.instructionPrompt) filledFields++;
        if (content.chainOfThought) filledFields++;
        if (content.fewShotExamples) filledFields++;
        if (content.validationCriteria) filledFields++;
        if (content.outputFormat) filledFields++;
