...
export class ExamplePatternLibrary {
    ...
    private async validatePattern(pattern: ExamplePattern): Promise<{
        ...
        if (pattern.quality < 0 || pattern.quality > 1) errors.push('Quality must be between 0 and 1');

        // Check metadata
        if (!pattern.metadata) {
            errors.push('Pattern metadata is required');
        } else {
            if (!pattern.metadata.domain) errors.push('Domain is required in metadata');
    ...
    private calculateQualityDistribution(scores: number[]): Record<string, number> {
        ...
        for (const score of scores) {
            if (score < 0.6) distribution.low++;
            else if (score < 0.8) distribution.medium++;
