...
class VibeWorkflowMCP {
    ...
    async handleFullWorkflow(args) {
        const { project_description, phases_to_run = ['all'], spawn_agents_at_phase = 7, agent_config = {} } = args;
    ...
    async handlePlanningPhase(args) {
    ...
    async handleExecutionPhase(args) {
    ...
    async executeVibeCommand(commandName, context) {
    ...
    extractTasksFromPlanningOutput(planningOutput) {
        // Similar to extractTasksFromPhases but for planning-specific output
        return this.extractTasksFromPhases(Object.entries(planningOutput).map(([name, result]) => ({
            name,
            result
        })));
    }
    async spawnExecutionAgents(tasks, agentConfig) {
    ...
    async executePhaseWithAgents(phase, tasks) {
        // Distribute phase-specific tasks to agents
        const phaseTasks = tasks.filter(task => this.isTaskForPhase(task, phase.number));
    ...
    async executeSingleAgentPhase(phase, context) {
    ...
    determineAgentRoles(projectPlan) {
    ...
    isTaskForPhase(task, phaseNumber) {
    ...
    buildUserPrompt(commandName, args) {
        const prompts = {
