...
import { getAgentRole } from '../common/config.js';
...
export class AgentManager extends EventEmitter {
    ...
    async spawnAgent(roleId, workingDirectory) {
    ...
    async startClaudeProcess(agent, workingDirectory) {
        const claudeArgs = [
            '--non-interactive',
...
`;
    ...
    setupProcessCommunication(agent, process) {
    ...
    handleTaskCompleted(agent, payload) {
    ...
    handleTaskFailed(agent, payload) {
    ...
    handleStatusUpdate(agent, payload) {
        if (payload.metadata) {
    ...
    async terminateAllAgents(force = false) {
        const terminationPromises = Array.from(this.agents.keys()).map(agentId => this.terminateAgent(agentId, force));
    ...
    getAvailableAgents() {
        return this.getAgents().filter(agent => agent.status === 'idle' &&
    ...
    async healthCheck() {
        ...
        const unhealthyAgents = agents.filter(agent => agent.status === 'error' ||
