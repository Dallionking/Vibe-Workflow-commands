...
export class ArchitectureAnalyzer {
    constructor() { }
    async analyzeArchitecture(architecture, focusAreas = []) {
        ...
        if (focusAreas.length > 0) {
            analysis.recommendations = analysis.recommendations.filter(rec => focusAreas.some(area => rec.toLowerCase().includes(area.toLowerCase())));
        }
        log.info(`Architecture analysis completed with score: ${analysis.score}/100`);
        return analysis;
    }
    analyzeComplexity(architecture) {
        const complexity = {
            overall: 0,
            frontend: 0,
            backend: 0,
        ...
        complexity.overall = Math.round((complexity.frontend * 0.25 +
            complexity.backend * 0.35 +
            complexity.database * 0.25 +
            complexity.deployment * 0.15));
        return complexity;
    }
    calculateFrontendComplexity(architecture) {
        let complexity = 1;
    ...
    calculateBackendComplexity(architecture) {
        let complexity = 1;
        // Base complexity from framework
        const frameworkComplexity = {
            'express': 2,
    ...
    calculateDatabaseComplexity(architecture) {
        let complexity = 1;
    ...
    calculateDeploymentComplexity(architecture) {
        let complexity = 1;
    ...
    analyzePerformance(architecture) {
        const performance = {
            score: 0,
            bottlenecks: [],
            optimizations: []
    ...
    calculateOverallScore(analysis) {
        ...
        return Math.round(complexityScore * weights.complexity +
            scalabilityScore * weights.scalability +
            securityScore * weights.security +
            performanceScore * weights.performance);
    }
    identifyStrengths(architecture) {
        const strengths = [];
    ...
    identifyWeaknesses(architecture) {
        const weaknesses = [];
    ...
    generateRecommendations(architecture, analysis) {
