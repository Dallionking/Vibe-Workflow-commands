...
import { Task, Agent, ProjectContext } from '../common/types.js';
...
class OrchestratorMCP {
  ...
  private setupEventListeners() {
    this.wsServer.on('agent_connected', (agentId: string) => {
      log.agent.spawned(agentId, 'unknown', 'pending');
    });

    this.wsServer.on('agent_disconnected', (agentId: string) => {
      this.agentManager.removeAgent(agentId);
    });

    this.wsServer.on('agent_message', (agentId: string, message: any) => {
      this.handleAgentMessage(agentId, message);
    });

    this.agentManager.on('agent_ready', (agent: Agent) => {
  ...
  private async handleSpawnAgents(args: any) {
    ...
    const taskObjects: Task[] = tasks.map((task: any) => ({
      id: generateId(),
      title: task.title,
    ...
    const spawnedAgents = await Promise.all(
      rolesToSpawn.map(async (role: string) => {
  ...
  private async handleTerminateAgents(args: any) {
    ...
    const terminationResults = await Promise.all(
      agentsToTerminate.map(async (agentId: string) => {
        try {
  ...
  private async handleCreateProjectContext(args: any) {
  ...
  private determineRequiredRoles(tasks: Task[]): string[] {
  ...
  private async distributeQueuedTasks() {
    const pendingTasks = this.taskQueue.getPendingTasks();
    const availableAgents = this.agentManager.getAgents().filter(agent => 
      agent.status === 'idle' && agent.currentTasks.length < agent.role.maxConcurrentTasks
    );
  ...
  private findSuitableAgent(agents: Agent[], task: Task): Agent | null {
    return agents.find(agent => {
      return agent.role.capabilities.some(capability => 
        task.domain.toLowerCase().includes(capability.toLowerCase()) ||
        task.description.toLowerCase().includes(capability.toLowerCase())
      );
    }) || null;
  }

  private async assignTaskToAgent(task: Task, agent: Agent) {
    // Update task status
    task.status = 'claimed';
  ...
  private async handleAgentMessage(agentId: string, message: any) {
    const agent = this.agentManager.getAgent(agentId);
    if (!agent) return;
  ...
  private async handleTaskCompleted(agentId: string, payload: any) {
  ...
  private async handleTaskFailed(agentId: string, payload: any) {
  ...
  private async handleStatusUpdate(agentId: string, payload: any) {
    const agent = this.agentManager.getAgent(agentId);
    if (!agent) return;
