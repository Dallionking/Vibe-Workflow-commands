...
class StoicMCP {
  ...
  private async handleGenerateTasks(args: any) {
  ...
  private async handleAnalyzeArchitecture(args: any) {
  ...
  private async handleEstimateEffort(args: any) {
  ...
  private async handleStoicPrompt(args: any) {
    const { project_description, constraints, preferences } = args;

    const constraintsList = constraints ? constraints.split(',').map((c: string) => c.trim()) : [];
...
Return the plan in a structured JSON format that can be used by the multi-agent system.`;
  ...
  private async handleArchitectureReviewPrompt(args: any) {
...
Return your analysis in a structured format.`;
  ...
  private async handleTaskBreakdownPrompt(args: any) {
...
Return the task breakdown in JSON format with task objects containing all required fields.`;
  ...
  private groupTasksByDomain(tasks: any[]): Record<string, number> {
  ...
  private async estimateProjectEffort(tasks: any[], teamSize: number = 1, experienceLevel: string = 'mid'): Promise<any> {
