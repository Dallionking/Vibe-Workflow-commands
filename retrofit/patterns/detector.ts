...
import Parser, { Tree, Node, Query, QueryCapture } from '../types/tree-sitter';
import {
  CodePattern,
  PatternType,
  SupportedLanguage,
...
export class PatternDetector {
  ...
  private async detectNamingPatterns(
    ...
    for (const [convention, examples] of Array.from(conventions.entries())) {
      if (examples.length < 2) {
        continue;
      } // Need minimum examples for confidence
  ...
  private async detectConventionPatterns(
    ...
    if (organizationPattern) {
      patterns.push(organizationPattern);
    }
  ...
  private analyzeComponentPatterns(node: Node, sourceCode: string, language: SupportedLanguage, filePath: string): CodePattern | null {
    ...
    if (componentIndicators.length === 0) {
      return null;
    }
  ...
  private detectQuotePattern(sourceCode: string): string {
    ...
    if (backticksQuotes > singleQuotes && backticksQuotes > doubleQuotes) {
      return 'backticks';
    }
